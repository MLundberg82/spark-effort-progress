...
function scoreAssetMatch(
  ...
  if (
    preferredFolder &&
    asset.path.toLowerCase().includes(preferredFolder.toLowerCase())
  ) {
    score += 80;
  }
...
export function getAllAssets(): AssetLookupResult[] {
  return indexedAssets.map(({ normalizedId, normalizedFileName, ...asset }) => asset);
}

export function getAssetById(id: string): AssetLookupResult | null {
...
export function getAssetSrc(id: string): string | null {
  return getAssetById(id)?.src ?? null;
}

export function getItemImage(id: string): string | null {
  return (
    getAssetSrc(id) ??
    getAssetSrc(`${id}.png`) ??
    getAssetSrc(`${id}.webp`) ??
...
export function getBackgroundImage(id: string): string | null {
  const hit =
...
export function getRatImage(id: string): string | null {
  const hit =
...
export function debugFindAssets(term: string) {
  return indexedAssets
    .map((asset) => ({
      ...
      path: asset.path,
      src: asset.src,
      score: scoreAssetMatch(asset, term),
    }))
    .filter((entry) => entry.score > 0)
    .sort((a, b) => b.score - a.score);
}