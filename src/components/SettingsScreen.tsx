...
import { getTrainingLevel, setTrainingLevel, type TrainingLevel } from '@/lib/trainingStore';
...
const SettingsScreen = ({ onBack, premiumActive = false }: Props) => {
  const [settings, setSettings] = useState<AppSettings>(getSettings());
  const [language, setLanguage] = useState<LanguageOption>(getSavedLanguage());
const [trainingLevelValue, setTrainingLevelValue] = useState<string>(
  getTrainingLevel?.() ?? 'beginner'
);
  ...
  const handleTrainingLevelChange = (value: TrainingLevel) => {
